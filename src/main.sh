...
for f in $(find . -maxdepth 2 -type f | sort | sed 's/\.\/src\///g'); do
    ...
    if [ "$CURRENT_HEADER" != "$DIRECTORY" ]; then
        PRINT_DIR=$(echo "$DIRECTORY" | sed -e 's/^\.\/.[0-9]-//g' -e 's/-/ /g' | awk '{for(i=1;i<=NF;i++){ $i=toupper(substr($i,1,1)) substr($i,2) }}1')
        printf "\n%s\n" "\newpage{}" >> ./main.md
        printf "\n# %s\n" "$PRINT_DIR" >> ./main.md
        CURRENT_HEADER=$DIRECTORY
    fi
    FILENAME=$(basename "$f" | awk '{for(i=1;i<=NF;i++){ $i=toupper(substr($i,1,1)) substr($i,2) }}1' | sed -e 's/\.md//g')
    {
        printf "\n%s\n" "## $FILENAME"
        printf "\n!include %s\n" "$f"
        printf "\n%s\n" "----"
